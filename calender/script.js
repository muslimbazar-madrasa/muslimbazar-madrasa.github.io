...
function loadXLSX(path){
  return fetch(path + '?v=' + Date.now())
    .then(resp => {
      if(!resp.ok) throw new Error(`${path} পাওয়া যায়নি (HTTP ${resp.status})`);
      return resp.arrayBuffer();
    })
    .then(buf => {
      const wb = XLSX.read(buf, {type:'array'});
      const sheet = wb.Sheets[wb.SheetNames[0]];
      // raw:false -> সংখ্যা/লেখা যেভাবে সেলে দেখা যায় সেভাবেই স্ট্রিং হিসেবে পড়বে
...
async function init(){
  ...
  try{
    ...
    console.error('dates.xlsx / events.xlsx লোড করতে সমস্যা হয়েছে:', e);
  }
...
function renderMonth(){
  ...
  weekdayNamesBn.forEach((w,i) => {
    ...
    h.className = 'wk-head' + (i===6?' fri':''); // শুক্রবার (শেষ কলাম) = সাপ্তাহিক ছুটি
    h.textContent = w;
    grid.appendChild(h);
  });

  const firstOfMonth = new Date(viewYear, viewMonth, 1);
  // আমাদের গ্রিড শনিবার (jsDay=6) থেকে শুরু হয়, তাই jsDay-কে শনিবার=0 ভিত্তিক ইনডেক্সে রূপান্তর
  const startDow = (firstOfMonth.getDay() + 1) % 7;
  ...
  for(let d=1; d<=daysInMonth; d++){
    ...
    if(dow===5) cell.classList.add('weekend'); // শুধু শুক্রবার সাপ্তাহিক ছুটি
  ...
  if(firstRow && lastRow){
    ...
    // dates.xlsx-এ এই মাসের ডেটা না থাকলে স্ট্রিপ দুটো শুধু লুকিয়ে রাখা হবে, বিরক্তিকর টেক্সট দেখানো হবে না
